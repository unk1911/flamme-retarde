...
**The slap, the kiss and her moans** — recorded by me, no third-party terms; cut by
...
**The water, and the Doodle's "hmm?"** — the two sound effects in this game
that are not either synthesised at runtime in `src/80-audio.js` or baked from
the owner's own recordings:
...
| `doodle_hmm.mp3` | the Slow Doodle's nose through the kabina curtain | [Hmm Ahh.wav](https://freesound.org/people/esperar/sounds/170781/) | esperar | CC0 1.0 |
...
is what that licence asks for. The "hmm?" is 0.88 s cut from the start of the
take, levelled to −18 dBFS RMS by `tools/cut_hmm.py`; CC0 asks for nothing.
