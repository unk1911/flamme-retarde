...
function buildAudio() {
   ...
   * SHE IS HER OWN VOICE NOW, AND THIS IS 1.0.
   *
   * The first cut of this played Balkanika 3.2 semitones up, because she was
   * the only female on the account labelled `language: hr` and she was already
   * cast as `woman_old` on the promenade. Every number said it worked — median
   * f0 209.6 against Jessica's 210.1, four hundredths of a semitone; 0.0 per
   * cent WER through speech-to-text; levels inside 0.4 dB; the hum's interval
   * preserved to four hundredths. Misha heard it: *"they all sound horrible,
   * like a chipmunk on speed speaking 100mph."*
   *
   * NOT ONE OF THOSE INSTRUMENTS CAN HEAR A CHIPMUNK. A pitch meter reports the
   * fundamental and a transcriber reports the words, and a resample moving the
   * formants with the pitch is audible to a person and invisible to both. That
   * is the lesson and it is worth more than the feature: a measurement can only
   * refute what it is able to notice.
   *
   * The fix was not a smaller shift, it was a second woman. The account's own
   * library was the wrong shelf: the SHARED library carries three hundred
   * voices tagged `hr` and adding one is a POST and a free slot. Three were
   * auditioned on six of her own lines at native pitch — Nina 181.8 Hz, Zlata
   * 195.2, Mila 210.6 — and Zlata was chosen, the most adopted Balkan female
   * there is. See `VOICE` in `tools/cut_mutter.py`.
   *
   * So the rate is 1.0 and it should stay 1.0. If she ever needs separating
   * from somebody again, separate her by CASTING and not by pitch: this whole
   * detour is what the other way costs. `voice_for`'s two children are still
   * an adult sped up, and that is a different case — a child IS a shorter vocal
   * tract, so moving the formants with the pitch is the right answer there and
   * the wrong one here.
   *
   * The hum is untouched and byte-identical, as it was before: it is Misha's
   * own tune, and `eleven_v3` can hold a note but only a melody of its own
   * invention.
   */
  const MUTTER = {
    rate: 1.0,
