...
def _bakeclip(rig, out, name, dur, src, f0, f1):
    """Write the whole window as a CLIP — a dense (t, pose) ladder.

    `--bake` indexes its keys by the depth they reach, because the stoop is
    asked for a depth. A routine is not: it is asked for a duration, and what
    has to survive is the TIMING — the counter-swing before a leg goes back,
    the pause at the top of a reach, the settle when the foot lands. That is
    the half of a capture a pose ladder throws away, and it is the half that
    six releases of hand-keying the stoop could not put back.
    So: one key per sampled frame, no interpolation of ours anywhere in it.
    """
    hmh = _hmh()
    keep = [h for h in MAP.values()]
    poses = []
    for _i, spec in out:
        poses.append(dict(hmh.IDLE_A,
                          **{k: v for k, v in spec.items() if k in keep}))
    # Feet on the deck, smoothed — which for a SAMPLED clip is the right way
    # round: see the note in `floor_poses`, where the staircase a frame-by-
    # frame solve leaves is an artefact, and only hand-placed keys want it
    # left alone.
    worst = hmh.floor_poses(rig, poses, passes=3)
    dur = dur or (len(poses) - 1) / float(hmh.SAMPLE_FPS)
    with open(name, 'w') as fh:
        fh.write('"""Generated by tools/blender/mocap_retarget.py --clip.'
                 '  DO NOT EDIT.\n\nCMU %s, frames %d..%d, %d keys, %.2f s.'
                 '  Deepest foot %+.3f m after the\nfloor pass.\n"""\n'
                 % (src, f0, f1, len(poses), dur, worst))
        fh.write('DUR = %.4f\n\nKEYS = [\n' % dur)
        for k, p in enumerate(poses):
            t = dur * k / max(1, len(poses) - 1)
            fh.write('    (%.4f, {\n' % t)
            fh.write('        "@root": (%.4f, %.4f, %.4f),\n' % tuple(p['@root']))
            for n in keep:
                if n in p:
                    fh.write('        "%s": (%.1f, %.1f, %.1f),\n'
                             % ((n,) + tuple(p[n])))
            fh.write('    }),\n')
        fh.write(']\n')
    print('[mocap] %s: %d keys, %.2f s, deepest foot %+.3f m'
          % (name, len(poses), dur, worst))
...
def main():
    ...
    if arg('--clip'):
        _bakeclip(rig, out, arg('--clip'), float(arg('--dur', '0')),
